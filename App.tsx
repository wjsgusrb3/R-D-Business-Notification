...
import { 
  Search, Building2, Flame, Calendar, ArrowRight, 
  CheckCircle2, ChevronRight, Bell, LayoutDashboard, 
...
// ─────────────────────────────────────────────
// Mock 데이터 (dDay는 런타임에 calcDDay로 계산)
// ─────────────────────────────────────────────
...
// AI 매칭 점수 계산 로직
// ─────────────────────────────────────────────
const calculateMatch = (userStr: string, grant: Grant): { score: number; reason: string; matches: string[] } => {
...
export default function App() {
  ...
  const [selectedGrant, setSelectedGrant] = useState<ProcessedGrant | null>(null);
  const [sortType, setSortType] = useState<SortType>('score'); // ← 정렬 상태 연결

  const [bookmarkedIds, setBookmarkedIds] = useState<number[]>([2, 4]);
  ...
  // ─────────────────────────────────────────────
  // 실시간 AI 공고 검색 (수정: gemini-2.0-flash 사용)
  // ─────────────────────────────────────────────
  const fetchRealTimeGrants = useCallback(async (currentCompany = companyName, currentKeywords = keywords) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          throw new Error("백엔드 서버에 연결할 수 없어 로컬 매칭 엔진으로 전환됩니다.");
        }
      }
      
      const data = await response.json();
      if (data.grants && data.grants.length > 0) {
        // 실시간 데이터에도 D-Day 동적 계산 적용
        const grantsWithDDay = data.grants.map((g: Grant) => ({ ...g, dDay: calcDDay(g.deadline) }));
        setGrants(grantsWithDDay);
      ...
      console.warn("실시간 로드 에러, 데모 모드로 전환:", msg);
      setApiError(msg);
      // Fallback: Mock 데이터 + D-Day 동적 계산
      const processedMock = MOCK_GRANTS.map(g => ({
        ...g,
        dDay: calcDDay(g.deadline),
  ...
  // 공고 목록 처리 (필터 + 정렬)
  // ─────────────────────────────────────────────
  const processedGrants = useMemo((): ProcessedGrant[] => {
    let results: ProcessedGrant[] = grants.map(grant => ({ 
      ...grant, 
      dDay: calcDDay(grant.deadline), // 렌더 시마다 최신 D-Day
      ...calculateMatch(keywords, grant) 
    }));
    ...
    if (sortType === 'score') {
      ...
      results.sort((a, b) => parseBudget(b.budget) - parseBudget(a.budget));
    }
    ...
    return results;
  }, [grants, keywords, showHighMatchOnly, activeTab, bookmarkedIds, searchQuery, sortType]);
  ...
  const handleConsultingSubmit = (e: React.FormEvent) => {
    ...
    // TODO: 실제 백엔드 API 연동 필요 (예: POST /api/consulting)
    setTimeout(() => {
      ...
      setConsultingPhone('');
      setConsultingText('');
      alert('전문가 매칭 상담 신청이 접수되었습니다. 담당 전문가가 24시간 이내에 연락드리겠습니다.');
    }, 1500);
  };
  ...
  const handleNewsletterSubmit = (e: React.FormEvent) => {
    ...
    // TODO: 실제 이메일 구독 API 연동 필요
    setTimeout(() => {
      setNewsletterSubscribed(false);
      setNewsletterEmail('');
      alert(`구독 완료! 매주 월요일 맞춤 R&D 정보를 ${newsletterEmail}로 보내드립니다.`);
    }, 1200);
  };

  // D-Day 임박 여부 판단 (수정: 숫자 비교로 D-30 오탐 방지)
  const isUrgent = (dDay: string): boolean => {
    const num = parseInt(dDay.replace('D-', '').replace('D+', '-').replace('D-Day', '0'));
    return !isNaN(num) && num <= 7 && num >= 0;
  };
  ...
  return (
    <div id="home_container" className="min-h-screen bg-[#f8fafc] font-sans text-slate-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      <header id="main_header" className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-xl font-extrabold tracking-tight text-blue-900">RD-CONNECT</span>
            ...
            <nav className="hidden md:flex items-center gap-1 text-sm font-medium text-slate-600">
              {([
                { tab: 'match', label: 'AI 맞춤추천' },
                { tab: 'all', label: '전체 통합공고' },
                { tab: 'favorites', label: '관심 공고 목록' },
              ] as const).map(({ tab, label }) => (
                <button 
                  key={tab}
                  onClick={() => setActiveTab(tab)} 
                  className={`px-4 py-2 rounded-lg transition-all duration-200 ${activeTab === tab ? 'text-blue-600 bg-blue-50/70 font-bold' : 'hover:text-blue-600 hover:bg-slate-50'}`}
                >
                  {label}
                </button>
              ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <div 
              onClick={() => { setEditCompanyName(companyName); setEditKeywords(keywords); setIsProfileModalOpen(true); }}
            ...
            >
              <div className="w-7 h-7 rounded-full bg-blue-600 text-white flex items-center justify-center font-extrabold text-xs shadow-xs">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        {/* 히어로 섹션 */}
        <section className="mb-8 bg-linear-to-r from-blue-900 to-indigo-950 rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-xl">
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
            <div className="md:col-span-8 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="bg-blue-500/20 text-blue-300 border border-blue-400/30 px-3 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1.5">
                  <Sparkles size={12} className="text-blue-400 animate-pulse" /> 
                  {isRealTimeActive ? "실시간 AI 공고 연동 중" : "지능형 매칭 엔진 작동 중"}
                </span>
                
                <button
                  onClick={() => fetchRealTimeGrants(companyName, keywords)}
                  disabled={isLoadingRealTime}
                  className="bg-white/10 hover:bg-white/20 text-white font-semibold text-xs px-2.5 py-1 rounded-full border border-white/20 transition-all active:scale-95 flex items-center gap-1 disabled:opacity-50"
                >
                  <RefreshCw size={10} className={isLoadingRealTime ? "animate-spin" : ""} />
                  {isLoadingRealTime ? "동기화 중..." : "실시간 공고 탐색"}
                </button>
              ...
              <p className="text-slate-300 text-sm md:text-base font-medium max-w-2xl">
                핵심 기술 키워드 <span className="text-white font-bold bg-blue-950/40 px-1.5 py-0.5 rounded border border-white/10">"{keywords}"</span> 기반으로 R&D 공고 {grants.length}건을 심층 분석했습니다.
              </p>
              ...
              {apiError && (
                <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg px-3 py-1.5 text-[11px] text-orange-300 flex items-center gap-1.5 max-w-xl">
                  ...
                  <span>API 미연결 상태로, 로컬 키워드 매칭 엔진으로 시뮬레이션 중입니다. (GEMINI_API_KEY 설정 시 완전 연동)</span>
                </div>
              )}
            </div>
            ...
            <div className="md:col-span-4 flex justify-end">
              <div className="bg-white/10 border border-white/10 rounded-2xl p-4 w-full md:max-w-xs backdrop-blur-md shadow-inner flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs text-blue-200 font-semibold mb-1">AI 적합 최우선 과제</p>
                  <p className="text-4xl font-extrabold tracking-tight text-white">{highMatchCount}건</p>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <aside className="lg:col-span-4 space-y-6">
            ...
            {/* 기업 현황 카드 */}
            <section className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs hover:shadow-md transition-shadow">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <p className="text-xs text-slate-500 mb-1 font-semibold">관심 공고</p>
            ...
            <section className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
              <div className="bg-slate-50/70 px-6 py-4 border-b border-slate-200/80">
                <h3 className="font-bold text-slate-800 flex items-center gap-2">
                  <Filter size={18} className="text-blue-600" /> 매칭 조건 필터
                </h3>
              </div>
              <div className="p-6 space-y-5">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">매칭 키워드</label>
                    <button 
                  ...
                  <textarea 
                    ...
                    className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-700 font-medium min-h-[90px] text-sm"
                    placeholder="예: 로봇, 정밀측정, 인공지능, 공정개선..."
                  />
                  <p className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">
                    * 쉼표(,)로 구분 입력 시 AI 적합도가 실시간 재계산됩니다.
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200">
                  <div className="flex flex-col text-left">
                    <span className="text-xs font-bold text-slate-800">80점 이상만 보기</span>
                    <span className="text-[10px] text-slate-500">AI 매칭 점수 기준</span>
                  </div>
            ...
            {/* 컨설팅 배너 */}
            <section className="bg-linear-to-br from-indigo-900 via-slate-900 to-indigo-950 rounded-2xl p-6 text-white relative overflow-hidden shadow-lg group">
              ...
              <div className="relative z-10 space-y-4">
                ...
                <div>
                  <p className="text-indigo-200 text-xs font-medium mb-1">사업계획서 작성이 막막하신가요?</p>
                  <h4 className="text-lg font-bold leading-snug">전문 컨설턴트 1:1 맞춤 매칭</h4>
                </div>
                <button 
                  onClick={() => setIsConsultingModalOpen(true)}
          ...
          <section className="lg:col-span-8 space-y-5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
              <div className="relative flex-1">
                ...
                <input 
                  ...
                  placeholder="사업명, 소관 부처, 기술 키워드로 검색..." 
                  className="w-full bg-slate-50 hover:bg-slate-100/50 focus:bg-white border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 rounded-xl pl-10 pr-4 py-2 text-sm transition-all focus:outline-none"
                />
                {searchQuery && (
                  <button 
              ...
              {/* 정렬 드롭다운 — 실제 sortType 상태와 연동 */}
              <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
                <span className="text-xs text-slate-400 font-semibold mr-1">정렬:</span>
                <select 
                  value={sortType}
                  onChange={(e) => setSortType(e.target.value as SortType)}
                  className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100 transition-colors focus:outline-none"
                >
                  <option value="score">추천 매칭 점수순</option>
                  <option value="deadline">공고 마감일 임박순</option>
                  <option value="budget">정부 지원 예산 높은순</option>
                </select>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {isLoadingRealTime ? (
                ...
                <div className="bg-white rounded-3xl border-2 border-dashed border-slate-200/80 py-20 px-6 text-center shadow-xs">
                  ...
                  <h4 className="text-slate-700 font-bold text-base mb-1">해당 조건에 맞는 R&D 사업이 없습니다</h4>
                  <p className="text-slate-400 text-sm max-w-md mx-auto">
                    {activeTab === 'favorites' 
                      ? '관심 있는 공고에 북마크(★)를 추가해 보세요.' 
                      : '키워드를 변경하거나 검색 내용을 초기화해 보세요.'}
                  </p>
                      ...
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex-1 space-y-3">
                          ...
                          <div className="pt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500">
                            <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-100 px-2.5 py-1 rounded-lg">
                              ...
                              {/* 수정: isUrgent()로 D-30 오탐 방지 */}
                              <span className={isUrgent(grant.dDay) ? 'text-red-600 font-bold' : 'font-medium'}>
                        ...
                        <div className="text-right shrink-0 flex flex-col items-end justify-between h-full min-h-[90px]">
                          <button 
                            ...
                            title="관심공고 보관"
                          >
                            <Bookmark size={18} className={bookmarkedIds.includes(grant.id) ? "fill-orange-400 text-orange-400" : "text-slate-400"} />
                          </button>
                          
                          <div className="mt-2 text-right">
                            <div className="text-[9px] font-bold text-slate-400 tracking-wider uppercase">AI 적합도</div>
                            <div className={`text-3xl font-black tracking-tight leading-none ${grant.score >= 80 ? 'text-blue-600' : 'text-slate-400'}`}>
            ...
            <button 
              onClick={() => alert('더 많은 R&D 과제를 보려면 전문 컨설팅 서비스를 이용해 주십시오.')}
      ...
      <footer className="mt-24 bg-white border-t border-slate-200 py-16">
        <div className="max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 md:grid-cols-12 gap-10">
          <div className="md:col-span-4 space-y-4">
            <div className="flex items-center gap-2">
              <div className="bg-slate-900 p-1.5 rounded-lg text-white"><LayoutDashboard size={16} /></div>
              <span className="font-extrabold text-slate-800 tracking-tighter text-lg">RD-CONNECT</span>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed max-w-xs">
              대한민국 강소중소기업을 위한 지능형 R&D 정보 수집 플랫폼입니다.
            </p>
            <p className="text-[10px] text-slate-400">* AI 분석 결과이므로 실제 공고 요강의 확인이 필수입니다.</p>
          </div>
          ...
          <div className="md:col-span-3 space-y-4">
            <h5 className="font-bold text-slate-800 text-xs uppercase tracking-wider">고객지원</h5>
            <ul className="text-xs text-slate-500 space-y-2.5">
              <li className="flex items-center gap-2"><Mail size={13} className="text-slate-400" /><span>support@rdconnect.kr</span></li>
              <li className="flex items-center gap-2"><Globe size={13} className="text-slate-400" /><span>1544-3000 (평일 9–18시)</span></li>
          ...
          <div className="md:col-span-3 space-y-4">
            ...
            <p className="text-xs text-slate-500 leading-normal">매주 우수 R&D 과제 동향 보고서를 무료로 받아보세요.</p>
            <form onSubmit={handleNewsletterSubmit} className="flex gap-2">
              <input 
                type="email" required value={newsletterEmail}
                onChange={(e) => setNewsletterEmail(e.target.value)}
                placeholder="이메일 주소" 
                className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs w-full focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" 
              />
              <button type="submit" disabled={newsletterSubscribed}
                className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 active:scale-95 text-white px-3 py-2 rounded-lg text-xs font-bold transition-all shrink-0">
                {newsletterSubscribed ? '처리중' : '구독'}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100 flex flex-col">
            <div className="p-6 border-b border-slate-100 flex items-start justify-between bg-white sticky top-0 z-10">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="bg-blue-100 text-blue-800 text-[10px] font-bold px-2 py-0.5 rounded">{selectedGrant.agency}</span>
                  <span className="bg-indigo-100 text-indigo-800 text-[10px] font-bold px-2 py-0.5 rounded">{selectedGrant.type}</span>
                </div>
                <h3 className="text-lg md:text-xl font-black text-slate-900 leading-snug">{selectedGrant.title}</h3>
              </div>
              <button onClick={() => setSelectedGrant(null)} className="text-slate-400 hover:text-slate-600 p-1.5 hover:bg-slate-100 rounded-full transition-colors active:scale-90">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6 flex-1 overflow-y-auto text-left">
              <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-blue-800 flex items-center gap-1.5">🤖 AI 핵심 일치성 분석</span>
                ...
                <p className="text-xs text-slate-700 leading-relaxed">{selectedGrant.reason}</p>
                <div className="flex flex-wrap gap-1.5">
                  <span className="text-[10px] font-semibold text-slate-400 mr-1 self-center">연관 키워드:</span>
                  {selectedGrant.matches && selectedGrant.matches.length > 0 ? (
                    ...
                    <span className="text-[10px] text-slate-400 italic">직접 연관 키워드가 없습니다.</span>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="border border-slate-200 rounded-xl p-3.5 bg-slate-50/30">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1">지원 한도</span>
                  <span className="text-sm font-extrabold text-slate-800">{selectedGrant.budget}</span>
                </div>
                <div className="border border-slate-200 rounded-xl p-3.5 bg-slate-50/30">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1">신청 마감</span>
                  <span className={`text-sm font-extrabold flex items-center gap-1 ${isUrgent(selectedGrant.dDay) ? 'text-red-600' : 'text-slate-800'}`}>
              ...
              {[
                { label: '지원 과제 개요', content: selectedGrant.description },
                { label: '신청 자격 요건', content: selectedGrant.eligibility },
                { label: '핵심 혜택', content: selectedGrant.benefit },
              ].map(({ label, content }) => (
                <div key={label} className="space-y-2">
                  <h5 className="font-bold text-sm text-slate-800 border-l-4 border-blue-600 pl-2">{label}</h5>
                  <p className="text-xs text-slate-600 leading-relaxed">{content}</p>
                </div>
              ))}

              <div className="space-y-2">
                <h5 className="font-bold text-sm text-slate-800 border-l-4 border-blue-600 pl-2">심사 일정</h5>
                <div className="bg-slate-50 rounded-xl p-3 text-xs text-slate-600 leading-relaxed">{selectedGrant.steps}</div>
              </div>
            </div>

            <div className="p-6 border-t border-slate-100 bg-white flex flex-col sm:flex-row items-center gap-3 justify-between sticky bottom-0 z-10">
              <div className="flex items-center gap-2">
                <button 
                  onClick={(e) => toggleBookmark(selectedGrant.id, e as unknown as React.MouseEvent)}
                ...
                >
                  <Bookmark size={14} className={bookmarkedIds.includes(selectedGrant.id) ? "fill-orange-400 text-orange-400" : "text-slate-400"} />
                  {bookmarkedIds.includes(selectedGrant.id) ? "관심공고 해제" : "관심공고 등록"}
                </button>
                <button 
                  onClick={() => alert('공고 링크가 복사되었습니다.')}
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button 
                  onClick={() => { setSelectedGrant(null); setIsConsultingModalOpen(true); }}
                ...
                <button 
                  onClick={() => { alert('공식 정부 포털(IRIS/SMTECH)로 이동합니다.'); setSelectedGrant(null); }}
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-100 flex flex-col text-left">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="font-extrabold text-slate-800 text-lg flex items-center gap-2">
                <Settings size={18} className="text-blue-600" /> 기업 매칭 프로필 설정
              </h3>
              <button onClick={() => setIsProfileModalOpen(false)} className="text-slate-400 hover:text-slate-600 p-1.5 hover:bg-slate-100 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleSaveProfile} className="p-6 space-y-4">
              <div>
                ...
                <input type="text" required value={editCompanyName} onChange={(e) => setEditCompanyName(e.target.value)}
                  className="w-full bg-slate-50 focus:bg-white border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none transition-all text-slate-700"
                  placeholder="예: (주)테스트엔지니어링" />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">핵심 기술 키워드 (쉼표 구분)</label>
                <textarea required value={editKeywords} onChange={(e) => setEditKeywords(e.target.value)}
                  className="w-full bg-slate-50 focus:bg-white border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none transition-all text-slate-700 min-h-[100px]"
                  placeholder="로봇, 정밀측정, 인공지능, 제조업" />
                <p className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">키워드 등록 시 AI 매칭 점수가 즉시 재계산됩니다.</p>
              </div>
              <div className="pt-4 border-t border-slate-100 flex items-center gap-3 justify-end">
                ...
                <button type="submit" className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs transition-colors shadow-md">
                  저장 및 AI 매칭 적용
                </button>
              </div>
            </form>
      ...
      {isConsultingModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-100 flex flex-col text-left">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <h3 className="font-extrabold text-slate-800 text-lg flex items-center gap-2">
                <Trophy size={18} className="text-yellow-500" /> R&D 전문가 1:1 상담 신청
              </h3>
              <button onClick={() => setIsConsultingModalOpen(false)} className="text-slate-400 hover:text-slate-600 p-1.5 hover:bg-slate-100 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleConsultingSubmit} className="p-6 space-y-4">
              <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-xs text-indigo-900 leading-relaxed">
                🧑‍🏫 <strong>무료 혜택:</strong> 합격 이력을 갖춘 전문 연구위원의 <strong>서면 매칭 자문 1회</strong>를 무상 지원합니다.
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" required value={consultingName} onChange={(e) => setConsultingName(e.target.value)}
                  className="w-full bg-slate-50 focus:bg-white border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none transition-all text-slate-700"
                  placeholder="예: 홍길동 팀장" />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">연락처</label>
                <input type="tel" required value={consultingPhone} onChange={(e) => setConsultingPhone(e.target.value)}
                  className="w-full bg-slate-50 focus:bg-white border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none transition-all text-slate-700"
                  placeholder="예: 010-1234-5678" />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">관심 분야 및 애로사항 (선택)</label>
                <textarea value={consultingText} onChange={(e) => setConsultingText(e.target.value)}
                  className="w-full bg-slate-50 focus:bg-white border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10 rounded-xl px-4 py-2.5 text-sm font-medium focus:outline-none transition-all text-slate-700 min-h-[80px]"
                  placeholder="예: 7월 IRIS 로봇 국산화 과제 사업계획서 작성 및 공동기관 매칭이 필요합니다." />
              </div>
              <div className="pt-4 border-t border-slate-100 flex items-center gap-3 justify-end">
                <button type="button" onClick={() => setIsConsultingModalOpen(false)}
                  className="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-500 font-bold text-xs hover:bg-slate-50 transition-colors">
                  취소
                </button>
                <button type="submit" disabled={consultingSubmitted}
                  className="px-5 py-2.5 rounded-xl bg-indigo-900 hover:bg-indigo-950 text-white font-bold text-xs transition-colors shadow-md flex items-center gap-1.5 disabled:bg-indigo-400">
                  {consultingSubmitted ? '제출 중...' : '컨설팅 무료 신청'}
                </button>
              </div>
            </form>
          </div>
